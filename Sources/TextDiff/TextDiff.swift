...
func diff(text1: String, text2: String,
    ...
    let commonSuffix = text1.commonSuffix(with: text2)
...
func halfMatchI(longText: String, longLength: Int, shortText: String, i: Int) -> HalfMatch? {
    ...
    while let offset = shortText.index(of: seed, from: currentOffset + 1) {
        let prefixLength = longText.substring(from: i).commonPrefix(with: shortText.substring(from: offset)).count
        let suffixLength = longText.substring(to: i).commonSuffix(with: shortText.substring(to: offset)).count
