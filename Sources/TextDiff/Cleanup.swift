            ...
            case .equal:
                ...
                if countDelete + countInsert > 1 {
                    if countDelete != 0 && countInsert != 0 {
                        ...
                        let prefix = textInsert.commonPrefix(with: textDelete)
                        if prefix.count > 0 {
                            ...
                            textInsert = textInsert.substring(from: prefix.count)
                            textDelete = textDelete.substring(from: prefix.count)
                        }
                        // Factor out any common suffixies.
                        let suffix = textInsert.commonSuffix(with: textDelete)
                        if suffix.count > 0 {
                            ...
                            diffs[pointer] = diff.with(text:
                                                        textInsert.dropFirst(insertLength - suffix.count)
                                                       + diff.text
                            )
                            
                            textInsert = textInsert.substring(to: insertLength - suffix.count)
                            textDelete = textDelete.substring(to: deleteLength - suffix.count)
                        }
                    }
...
func cleanupSemanticLossless(diffs: [Diff]) -> [Diff] {
    ...
    while pointer < diffs.count - 1 {
           ...
           case .equal(var equality2) = diffs[pointer + 1]
        {
            ...
            let commonString = equality1.commonSuffix(with: edit)
            if commonString.count > 0 {
                let editLength = edit.count
                let commonOffset = commonString.count
...
public func cleanupSemantic(diffs: [Diff]) -> [Diff] {
    ...
    while pointer < diffs.count {
           ...
           case .insert(let insertion) = diffs[pointer]
        {
            ...
            let overlapLength1 = deletion.commonOverlapLength(with: insertion)
            let overlapLength2 = insertion.commonOverlapLength(with: deletion)
